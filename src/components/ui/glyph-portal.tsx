...
export type GlyphPortalProps = {
  ...
  children?: ReactNode;
  scrollLength?: number;
  /** Viewport-height units of scroll with no zoom after the section engages. */
  scrollLeadVh?: number;
  fontFamily?: string;
  fontWeight?: number;
...
export default function GlyphPortal({
  ...
  useLayoutEffect(() => {
    ...
    const clip = section.querySelector<SVGClipPathElement>(`#${CSS.escape(clipId)}`)!;
    ...
    const position = () => {
      ...
      const lead = clamp(scrollLeadVh, 0, 3) * H;
      const pinTop = pin.getBoundingClientRect().top;
      if (pinTop > origin + 1) return 0;
      return clamp((origin - section.getBoundingClientRect().top - lead) / travel);
    };

    const paint = (rawProgress: number) => {
      const isStatic = motion.matches || !browserFrameSeen || stalled || !target;
      const p = isStatic ? 0 : rawProgress;
    ...
    const layout = () => {
      ...
      const leadVh = clamp(scrollLeadVh, 0, 3);
      section.style.setProperty(
        "--gp-scroll-lead",
        leadVh > 0 ? `${leadVh * 100}svh` : "0px",
      );
      travel = H * length;
    ...
    const visibility = new IntersectionObserver(
      ([entry]) => {
        ...
        if (active) {
          dirty = true;
          if (!loopRaf) loopRaf = requestAnimationFrame(runLoop);
          schedule();
        } else {
          if (loopRaf) {
    ...
    >
      ...
      <div data-gp-pin>
        ...
        <div data-gp-caption>
          ...
          <a data-gp-enter href={`#${sectionId}-content`}>
            {enterLabel}
            <span aria-hidden="true">↘</span>
          </a>
        </div>
      </div>
      <div data-gp-content id={`${sectionId}-content`} tabIndex={-1}>
        {children ?? (
          <div>
